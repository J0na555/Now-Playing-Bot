// Caption builders for Telegram (HTML parse mode) and the small helpers that
// decide which card is showing right now.

export function escapeHtml(s: string): string {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

// Spotify is "actively playing" only when the widget says "Now playing" AND the
// progress time is moving between polls (a frozen progress bar means paused).
// First observation fails open: with no lastProgress to compare we assume playing.
